...
```vim
# 安装jekyll（OS X 10.11以前）
$ sudo gem install jekyll
# 安装jekyll（OS X 10.11后苹果升级了安全策略）
$ sudo gem install -n /usr/local/bin jekyll
```

### 卸载Jekyll

如果突然有一天不想用Jekll，想卸载掉，那么就执行如下命令：  
...
> NOTE： Jekyll 3.3.0 之后开始采用Bundler方式运行。我们只需要安装Bundler，然后通过Bundler来安装运行Jekyll。
...
`bundler install`命令会主动把Jekyll以及其他的一些依赖包安装好，无需我们手动安装Jekyll。当一切都安装好以后，我们只需要在原来Jekyll命令的基础上加上前缀`bundle exec `即可运行，事例如下：  
